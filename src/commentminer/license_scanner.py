...
def scan_comment_licenses(
    ...
    *,
    ...
    input_shards_by_name = {path.name: path for path in input_shards}
    for shard_name in sorted(completed_shards):
        input_shard = input_shards_by_name.get(shard_name)
        output_shard = output_directory / shard_name
        shard_stats = checkpoint.shard_stats.get(shard_name)
        if (
            input_shard is None
            or not _checkpoint_source_matches(input_shard, shard_stats)
            or not _jsonl_output_shard_has_license_columns(
                output_shard,
                expected_records=shard_stats.get("records_scanned") if shard_stats else None,
            )
        ):
            completed_shards.discard(shard_name)
            checkpoint.shard_stats.pop(shard_name, None)
            checkpoint_changed = True
            continue
        assert shard_stats is not None
        stats.records_scanned += shard_stats["records_scanned"]
        stats.records_with_detected_license += shard_stats[
            "records_with_detected_license"
        ]
        stats.records_without_detected_license += (
            shard_stats["records_scanned"]
            - shard_stats["records_with_detected_license"]
        )
    ...
    next_progress_update = (
        (stats.records_scanned // progress_every) + 1
    ) * progress_every

    _LOGGER.info(
        "Starting comment license scan input_directory=%s output_directory=%s batch_size=%s scanner_backend=%s scancode_command=%s",
        input_directory,
        output_directory,
        batch_size,
        scanner_backend,
    ...
    try:
        for input_shard in input_shards:
            ...
            temp_output_shard = output_shard.with_name(
                f"{output_shard.name}.tmp.{os.getpid()}"
            )
            with input_shard.open("r", encoding="utf-8") as source_handle, temp_output_shard.open(
                "w", encoding="utf-8"
            ) as output_handle:
            ...
            temp_output_shard.replace(output_shard)

            completed_shards.add(shard_name)
            checkpoint.completed_shards = sorted(completed_shards)
            checkpoint.shard_stats[shard_name] = {
                "records_scanned": records_in_shard,
                "records_with_detected_license": detections_in_shard,
                "batches_run": math.ceil(records_in_shard / batch_size),
        ...
        for temp_output_shard in output_directory.glob("part-*.jsonl.tmp.*"):
            temp_output_shard.unlink(missing_ok=True)
        if temp_root.exists():
    ...
    *,
    ...
    if jsonl_shards:
        ...
        for shard in jsonl_shards:
            _add_jsonl_license_scores_to_histogram(
                histogram,
                shard,
                lower_score=lower_score,
                upper_score=upper_score,
...
def _add_jsonl_license_scores_to_histogram(
    histogram: LicenseScoreHistogram,
    shard: Path,
    *,
    lower_score: float,
    ...
    with shard.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if dataset_filter and str(payload.get("dataset")) not in dataset_filter:
                continue
            if language_filter and str(payload.get("language")) not in language_filter:
                continue
            histogram.records_seen += 1
            _add_score_to_histogram(
                histogram,
                payload.get("comment_license_score"),
                lower_score=lower_score,
                upper_score=upper_score,
            )


def _jsonl_output_shard_has_license_columns(
    output_shard: Path,
    *,
    expected_records: int | None,
) -> bool:
    if not output_shard.exists() or not output_shard.is_file():
        return False
    try:
        with output_shard.open(encoding="utf-8") as handle:
            for line in handle:
                if not line.strip():
                    continue
                payload = json.loads(line)
                return (
                    ...
                    and isinstance(payload.get("comment_license_detection"), dict)
                    and "comment_license_score" in payload
                )

...
Use `dev/agent` for agent-assisted development, experiments, planning artifacts, and any AI-specific support files.

Treat `main` as a promotion branch only:

- only merge or cherry-pick production-ready code
- do not commit agent scratch files, exploratory notes, or workflow artifacts
- move changes from `dev/agent` to `main` only after they are reviewed and ready
...
- a storage-aware pipeline skeleton
- checkpoint and shard writing utilities
- docs for the main design constraints

The next implementation step is to add concrete dataset adapters for sources such as The Stack and The Stack v2, followed by an `ml4se-tk` integration layer.
